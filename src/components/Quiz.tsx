...
export default function Quiz({ questions, onComplete, onFeedbackSubmit, className }: QuizProps) {
  ...
  const isCurrentQuestionAnswered = currentQuestionId ? !!answers[currentQuestionId] : false;
  
  const correctAnswers = Object.values(answers).filter(a => a.isCorrect).length;

  const handleAnswered = (questionId: string, answerId: string, isCorrect: boolean) => {
    setAnswers((prev) => ({
      ...prev,
    ...
    return (
      <section className={twMerge("flex flex-col flex-1 mt-6", className)}>
        <div className="text-center mb-[3.75rem]">
          ...
          <p className="text-white/80 ">You've completed today's DTE. Your <br /> metrics have now changed!</p>
        </div>
          ...
          <div className="relative z-10">
            ...
            <div className="w-full flex flex-col space-y-1.5 font-medium text-sm border-b pb-1 mb-1.5 border-b-white/20">
              ...
              {questions.length === correctAnswers && (
                <div className="flex flex-row justify-between">
                  <span className="text-white/80">Correct Knowledge Check Answer</span>
                  +15
                </div>
              )}
            </div>
